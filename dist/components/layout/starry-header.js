...
export const StarryHeaderBackground = ({ width = 430, height = 136, backgroundColor = "#171A17", starColor = "#FBCB4A", className, }) => {
    const containerRef = useRef(null);
    const starfieldRef = useRef(null);
    const allStarsRef = useRef([]);
    const activeShootingStarsRef = useRef([]);
    ...
    // --- Star Creation Logic ---
    const createStarInLayer = useCallback((starfield, stars, layerClass, minSize, maxSize, minOpacity, maxOpacity, returnData = false, forceX = null, forceY = null) => {
    ...
    }, [width, height, starColorClass]); // Dependencies for star creation
    const createShootingStar = useCallback((starfield) => {
        ...
        const speed = SHOOTING_STAR_SPEED_MIN +
    ...
    }, [width, height, shootingStarColorClass]); // Dependencies for shooting star creation
    // --- Initialization ---
    useEffect(() => {
        ...
        const containerStars = []; // Temporary array for this container
        // Create foreground stars
        for (let i = 0; i < FOREGROUND_STARS; i++) {
            createStarInLayer(starfield, containerStars, "foreground", 2.0, 3.5, 0.7, 1.0);
        }
        // Create middle stars
        for (let i = 0; i < MIDDLE_STARS; i++) {
            createStarInLayer(starfield, containerStars, "middle", 1.5, 2.8, 0.5, 0.8);
        }
        // Create background stars with clustering
        let currentBackgroundStars = 0;
        while (currentBackgroundStars < BACKGROUND_STARS) {
            const primaryStarData = createStarInLayer(starfield, containerStars, "background", 0.5, 1.3, 0.3, 0.5, true);
            if (!primaryStarData)
                continue; // Should not happen if returnData=true
            ...
            if (Math.random() < CLUSTER_CHANCE &&
                ...
                for (let j = 0; j < STARS_PER_CLUSTER; j++) {
                    ...
                    createStarInLayer(starfield, containerStars, "background", 0.4, 1.0, 0.25, 0.45, false, clusterX, clusterY);
                    currentBackgroundStars++;
                }
            }
            else {
    ...
    const animate = useCallback(() => {
        ...
        allStars.forEach((star) => {
            ...
            const influenceX = Math.max(-1, Math.min(1, mouseOffsetX / influenceRangeX));
            const influenceY = Math.max(-1, Math.min(1, mouseOffsetY / influenceRangeY));
            ...
            newOpacity = Math.max(baseOpacity * minOpacityFactor, Math.min(baseOpacity * maxOpacityFactor, newOpacity));
        ...
        if (currentShootingStarCount < MAX_SHOOTING_STARS &&
    ...
    return (_jsxs("div", { ref: containerRef, className: cn("starry-container", "relative overflow-hidden rounded-[10px]", className), style: {
            width: `${width}px`,
            height: `${height}px`,

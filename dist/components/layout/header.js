...
export function LayoutHeader({ onBack, onClose, hideUsername, hideNetwork, hideSettings, onOpenStarterPack, ...innerProps }) {
    ...
    const coverUrl = useMemo(() => {
        return getComputedStyle(document.documentElement).getPropertyValue("--theme-cover-url");
    }, []);
    ...
    return (_jsxs("div", { className: "sticky top-0 w-full z-10 bg-background", children: [(() => {
            ...
            })(), _jsxs("div", { className: "flex items-center justify-between absolute top-0 left-0 right-0 h-16 p-2 z-50", children: [_jsx("div", { children: onBack ? (_jsx(BackButton, { onClick: onBack })) : shouldShowCloseButton ? (_jsx(CloseButton, { onClose: () => {
                                if (onClose)
                                    onClose();
                                if (closeModal)
                                    closeModal();
                            } })) : null }), _jsxs("div", { className: "flex items-center gap-2", children: [!!chainId &&
                                (account ? (_jsxs(_Fragment, { children: [onOpenStarterPack && (_jsxs(Button, { variant: "secondary", size: "default", className: "gap-2", onClick: onOpenStarterPack, children: [_jsx(GiftIcon, { size: "default", variant: "line" }), _jsx("span", { children: "Get Starter Pack" })] })), _jsx(ConnectionTooltip, { username: account.username, address: account.address, chainId: chainId, followers: followers, followings: followings, hideNetwork: hideNetwork, hideUsername: hideUsername, onFollowersClick: onFollowersClick, onFollowingsClick: onFollowingsClick, onOpenSettings: openSettings, onLogout: onLogout })] })) : (!hideNetwork && _jsx(Network, { chainId: chainId }))), openSettings && !hideSettings && (_jsx(SettingsButton, { onClick: openSettings }))] })] })] }));
}
export function HeaderInner({ variant, Icon, icon, title, description, right, className, hideIcon, }) {
    if (variant === "hidden")
        return null;
    return (_jsxs("div", { className: cn("p-6 pb-0 flex items-center justify-between", className), children: [_jsxs("div", { className: "flex items-center flex-shrink min-w-0 gap-3", children: [!hideIcon && (_jsx(HeaderIcon, { variant: variant, Icon: Icon, icon: icon })), _jsx(Headline, { variant: variant, title: title, description: description })] }), right] }));
}
export function HeaderIcon({ variant, Icon, icon, }) {
    return (_jsx(IconWrapper, { variant: variant, children: (() => {
            ...
            const iconUrl = useMemo(() => getComputedStyle(document.documentElement).getPropertyValue("--theme-icon-url"), []);
            if (Icon) {

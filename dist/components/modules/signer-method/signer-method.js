...
export function SignerMethod({ className, kind, onClick }) {
    ...
    return (_jsxs("div", { className: cn("flex items-center", "w-full h-[40px] p-2.5 gap-2", "rounded", "bg-background-200 hover:bg-background-300", "text-foreground-100 font-ld text-sm text-normal uppercase tracking-[2.1px]", "cursor-pointer transition-colors ease-in-out", className), onClick: onClick, children: [icon && _jsx("div", { className: cn("w-fit h-fit"), children: icon }), label || kind.charAt(0).toUpperCase() + kind.slice(1)] }));
}

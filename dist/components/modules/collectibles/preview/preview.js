...
export const CollectiblePreview = ({ image, totalCount, listingCount, variant, size, className, ...props }) => {
    ...
    return (_jsxs("div", { className: cn(collectiblePreviewVariants({ variant, size }), className), ...props, children: [_jsxs("div", { className: "absolute grow inset-0 blur-[8px] transition-opacity duration-150 opacity-75 group-hover:opacity-100", children: [_jsx("img", { src: data || uri, className: cn("object-cover absolute inset-0 w-full h-full image-pixelated") }), _jsx("div", { className: "bg-center bg-cover h-full w-full relative", style: {
                            backgroundImage: `linear-gradient(0deg, rgba(0, 0, 0, 0.64), rgba(0, 0, 0, 0.64))`,
                        } })] }), _jsx("img", { className: "object-contain h-full w-full relative transition duration-150 ease-in-out group-hover:scale-[1.02]", style: { imageRendering: "pixelated" }, draggable: false, src: data || image, onError: (e) => {
                    e.currentTarget.src = PLACEHOLDER;
                } }), _jsxs("div", { className: "flex gap-1 items-center flex-wrap justify-start absolute bottom-1.5 left-1.5", children: [!!totalCount && (_jsx(CollectibleTag, { label: `${formatNumber(totalCount)}`, children: _jsx(StackDiamondIcon, { variant: "solid", size: "sm" }) })), !!listingCount && (_jsx(CollectibleTag, { label: `${formatNumber(listingCount)}`, children: _jsx(TagIcon, { variant: "solid", size: "sm" }) }))] })] }));
};

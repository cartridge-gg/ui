...
export const TokenCard = ({ image, title, amount, increasing, decreasing, value, change, clickable = true, roundedImage = true, isFree = false, variant, className, ...props }) => {
    const [hover, setHover] = useState(false);
    const Logo = useMemo(() => (_jsx(Thumbnail, { icon: image, size: "lg", variant: hover ? "lighter" : "light", rounded: roundedImage })), [image, hover]);
    const style = useMemo(() => {
    ...
    return (_jsx(ActivityCard, { Logo: Logo, title: title, subTitle: Amount, topic: value, subTopic: Change, badge: isFree ? "FREE" : undefined, variant: variant, className: cn("rounded-none", !clickable && "pointer-events-none", className), style: style, onMouseEnter: () => setHover(true), onMouseLeave: () => setHover(false), ...props }));
};

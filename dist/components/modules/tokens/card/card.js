...
export const TokenCard = ({ image, title, amount, value, change, variant, className, ...props }) => {
    const Logo = useMemo(() => _jsx(Thumbnail, { icon: image, size: "lg", variant: "light", rounded: true }), [image]);
    const style = useMemo(() => {
        if (change?.includes("+")) {
            return {
    ...
    return (_jsx(ActivityCard, { Logo: Logo, title: title, subTitle: amount, topic: value, subTopic: Change, variant: variant, className: cn("rounded-none", className), style: style, ...props }));
};

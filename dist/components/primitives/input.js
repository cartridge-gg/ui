...
const Input = React.forwardRef(({ error, isLoading, onClear, variant, size, className, type, ...props }, ref) => {
    const [isFocused, setIsFocused] = React.useState(false);
    const [isHovered, setIsHovered] = React.useState(false);
    return (_jsxs("div", { className: "flex flex-col gap-y-3", children: [_jsxs("div", { className: "relative", onMouseEnter: () => setIsHovered(true), onMouseLeave: () => setIsHovered(false), onFocus: () => setIsFocused(true), onBlur: () => setIsFocused(false), children: [_jsx("input", { ref: ref, type: type, className: cn(inputVariants({ variant, size, className }), !!props.value && !!onClear && "pr-12", !!error &&

...
export interface RadialToggleProps extends HTMLAttributes<HTMLDivElement>, VariantProps<typeof radialToggleVariants> {
  active?: boolean;
}

export const RadialToggle = React.forwardRef<HTMLDivElement, RadialToggleProps>(
  (
    {active, className, variant, size },
    ref,
  ) => {
    return (
      <div
      ...
      >
        <div className="w-full h-full rounded-full flex items-center justify-center p-0.5 border border-background-200 bg-spacer-100">
          <div data-active={active} className="w-full h-full rounded-full bg-transparent group-hover:bg-background-500 data-[active=true]:bg-primary-100 data-[active=true]:group-hover:bg-primary-100 transition-colors" />
        </div>
      </div>
    );
  },
